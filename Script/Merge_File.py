        ...
        for file_name in os.listdir(path):
            ...
            if os.path.isfile(file_path):
                with open(file_path, 'r', encoding='utf8') as in_f:
                    lines = [line.strip().replace(' ','').replace('HOST,','DOMAIN,').replace.replace('host,','DOMAIN,')('HOST-SUFFIX','DOMAIN-SUFFIX').replace('host-suffix','DOMAIN-SUFFIX').replace('host-keyword','DOMAIN-KEYWORD').replace('HOST-KEYWORD','DOMAIN-KEYWORD') for line in in_f.readlines() if not line.startswith("#")]
