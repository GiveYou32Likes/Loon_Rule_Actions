        ...
        for file_name in os.listdir(path):
            ...
            if os.path.isfile(file_path):
                with open(file_path, 'r', encoding='utf8') as in_f:
                    lines = [line.strip().replace(' ','').replace(',reject','').replace('host,','DOMAIN,').replace('host-','DOMAIN-').replace('HOST,','DOMAIN,').replace('HOST-,','DOMAIN-,') for line in in_f.readlines() if not line.startswith("#")]
